...
contract Lottery645 is LotteryOneInterface, AutomationCompatibleInterface, ReentrancyGuard {
    ...
    // Lottery name
    string public  name = 'lottery645';
    // Minimum number of numbers users must pick to buy a ticket
    uint constant MUSTPICKEDNUMBERS = 6;
    ...
    function enter(uint[] memory _pickedNumbers) external payable {
        ...
        for (uint i = 0; i < MUSTPICKEDNUMBERS; i++) {
            require(_pickedNumbers[i] >= 1, 'Picked numbers must be from 1 to 45');
            require(_pickedNumbers[i] <= MAXNUMBER, 'Picked numbers must be from 1 to 45');
    ...
    function withdrawWinnings(uint _ticketId) external nonReentrant {
        ...
        if ((matchedCount >= 2) && (matchedCount < 6)) {
    ...
    constructor(address _governance) payable {
        ...
        prizeMultipliers[3] = 3;
        prizeMultipliers[4] = 30;
        prizeMultipliers[5] = 2000;
        // 6 = Jackpot only
    }
