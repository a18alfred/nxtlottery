...
contract Lottery420 is LotteryOneInterface, AutomationCompatibleInterface, ReentrancyGuard {
    ...
    // Lottery name
    string public  name = 'lottery420';
    // Minimum number of numbers users must pick to buy a ticket
    uint constant MUSTPICKEDNUMBERS = 8;
    ...
    mapping(uint => mapping(uint => uint)) internal prizeMultipliers;
    ...
    // Mapping lotteryId => => Field number => drawn number => true if it was drawn
    mapping(uint => mapping(uint => mapping(uint => bool))) internal isDrawnNumber;
    ...
    function enter(uint[] memory _pickedNumbers) external payable {
        ...
        // Checking uniqueness and that numbers in range 1-MAXNUMBER (Field 1)
        for (uint i = 0; i < 4; i++) {
            require(_pickedNumbers[i] >= 1, 'Picked numbers must be from 1 to 20');
            require(_pickedNumbers[i] <= MAXNUMBER, 'Picked numbers must be from 1 to 20');
            if (i > 0) require(_pickedNumbers[i] > _pickedNumbers[i - 1], 'Numbers must be sorted');
            bool isUnique = true;

            for (uint n = i + 1; n < 4; n++) {
                if (_pickedNumbers[i] == _pickedNumbers[n]) {
                    isUnique = false;
                }
            }
            require(isUnique, 'Every picked number must be unique in the field 1');
        }

        // Checking uniqueness and that numbers in range 1-MAXNUMBER (Field 2)
        for (uint i = 4; i < 8; i++) {
            require(_pickedNumbers[i] >= 1, 'Picked numbers must be from 1 to 20');
            require(_pickedNumbers[i] <= MAXNUMBER, 'Picked numbers must be from 1 to 20');
            if (i > 4) require(_pickedNumbers[i] > _pickedNumbers[i - 1], 'Numbers must be sorted');
            bool isUnique = true;
            ...
            require(isUnique, 'Every picked number must be unique in the field 2');
        }
    ...
    function getLotteryById(uint _lotteryId) external view returns (
        ...
        if (lottery[_lotteryId].state != LOTTERY_STATE.OPEN) {
            uint[] memory tempArr = new uint[](8);
            uint numbersCount = 0;

            for (uint i = 1; i <= MAXNUMBER; i++) {
                if (isDrawnNumber[_lotteryId][1][i]) {
                    tempArr[numbersCount] = i;
                    numbersCount += 1;
                    if (numbersCount == 4) break;
                }
            }

            for (uint i = 1; i <= MAXNUMBER; i++) {
                if (isDrawnNumber[_lotteryId][2][i]) {
                    tempArr[numbersCount] = i;
                    numbersCount += 1;
                    if (numbersCount == 8) break;
                }
            }
    ...
    function fulfill_drawing(uint randomness) external {
        ...
        while (drawCount < 4) {
            ...
            if (isUnique) {
                ...
                isDrawnNumber[lotteryId][1][number] = true;
                drawCount += 1;
            }
            secondArg++;
        }
        ...
        while (drawCount < 8) {
            ...
            if (isUnique) {
                drawnNumbers[drawCount] = number;
                isDrawnNumber[lotteryId][2][number] = true;
                drawCount += 1;
            }
            secondArg++;
        }
    ...
    function withdrawWinnings(uint _ticketId) external nonReentrant {
        ...
        uint matchedCountF1 = 0;
        for (uint n = 0; n < 4; n++) {
            if (isDrawnNumber[ticketLotteryId][1][pickedNumbers[n]]) matchedCountF1 += 1;
        }

        uint matchedCountF2 = 0;
        for (uint n = 4; n < 8; n++) {
            if (isDrawnNumber[ticketLotteryId][2][pickedNumbers[n]]) matchedCountF2 += 1;
        }

        require(((matchedCountF1 >= 2) || (matchedCountF2 >= 2)), 'Your ticket lost');

        // Depending on how many numbers matched we will determine what tickets won/lost and how much they won
        if ((matchedCountF1 == 4) && (matchedCountF2 == 4)) {
            paidOut = lottery[ticketLotteryId].jackpot.div(sameCombinationCounter);
            growingJackpot = 0;
        } else {
            if (matchedCountF1 >= matchedCountF2) {
                paidOut = prizeMultipliers[matchedCountF1][matchedCountF2] * lottery[ticketLotteryId].minimumPrice;
                paidOut = paidOut.div(10);
            } else {
                paidOut = prizeMultipliers[matchedCountF2][matchedCountF1] * lottery[ticketLotteryId].minimumPrice;
                paidOut = paidOut.div(10);
            }
        }
    ...
    constructor(address _governance) payable {
        ...
        // Exclusion: 4 4 matched numbers get JACKPOT (if many users won jackpot then jackpot splits equally)
        // Exclusion: 0 or 1 matched numbers means the ticket lost
        // 13 means 1.3. All number should be divided by 10
        prizeMultipliers[2][0] = 13;
        prizeMultipliers[2][1] = 10;
        prizeMultipliers[2][2] = 32;
        prizeMultipliers[3][0] = 46;
        prizeMultipliers[3][1] = 42;
        prizeMultipliers[3][2] = 142;
        prizeMultipliers[3][3] = 313;
        prizeMultipliers[4][0] = 717;
        prizeMultipliers[4][1] = 2038;
        prizeMultipliers[4][2] = 2085;
        prizeMultipliers[4][3] = 15083;
        // 4 4 = Jackpot only
    }
    ...
    //        // getting multiple random numbers
    //        isDrawnNumber[lotteryId][1][1] = true;
    //        isDrawnNumber[lotteryId][1][2] = true;
    //        isDrawnNumber[lotteryId][1][3] = true;
    //        isDrawnNumber[lotteryId][1][4] = true;
    //        isDrawnNumber[lotteryId][2][1] = true;
    //        isDrawnNumber[lotteryId][2][2] = true;
    //        isDrawnNumber[lotteryId][2][3] = true;
    //        isDrawnNumber[lotteryId][2][4] = true;
    //
    //        // Closing currect lottery draw
    //        lottery[lotteryId].state = LOTTERY_STATE.CLOSED;
    //
