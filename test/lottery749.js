...
contract('Lottery749', ([deployer, ...accounts]) => {
	let lottery749, governance, tickets;
	let lotteryAddress;
	...
	before(async () => {
		...
		lotteryAddress = await lottery749.address;
	});

	describe('deployment', async () => {
		it('deploys successfully', async () => {
			const address = await lottery749.address;
		...
		it('has correct initial values', async () => {
			const lotteryId = await lottery749.lotteryId();
			const growingJackpot = await lottery749.growingJackpot();
	...
	describe('starting a new lottery', async () => {
		it('started a new lottery draw with correct inital value', async () => {
			...
			let pickedNumbers = [1, 2, 3, 4, 5, 6, 7];
			await lottery749.enter(pickedNumbers, {
				from: accounts[0],
				value: minimumPrice
			}).should.be.rejected;

			const receipt = await lottery749.manual_start_new_lottery();
			const lotteryId = await lottery749.lotteryId();
			assert.equal(lotteryId, 1, 'lotteryId must be 0');
			const lottery1 = await lottery749.getLotteryById(lotteryId);
			...
			const lotName = await lottery749.name();
	...
	describe('protection', async () => {
		it('rejects external calls', async () => {
			await lottery749.fulfill_drawing(1).should.be.rejected;
			await lottery749.fulfill_drawing(1, { from: deployer }).should.be.rejected;
			await lottery749.fulfill_drawing(1, { from: accounts[0] }).should.be.rejected;
			await lottery749.manual_start_new_lottery({ from: accounts[0] }).should.be.rejected;
		});
	});
	...
	describe('tickets', async () => {
		it('buying tickets', async () => {
			...
			await lottery749.enter([45, 3, 4, 5, 6, 7], { from: accounts[0], value: minimumPrice }).should.be.rejected;
			await lottery749.enter([45, 3, 4, 5, 6, 7, 8], {
				from: accounts[0],
				value: minimumPrice
			}).should.be.rejected;
			...
			for (let i = 1; i <= 5; i++) {
				...
				let gasPrice = await lottery749.enter.estimateGas(pickedNumbers, {
					from: accounts[0],
					value: minimumPrice
				});
				console.log('gasPrice = ', gasPrice);
				const receipt = await lottery749.enter(pickedNumbers, { from: accounts[0], value: minimumPrice });
			...
			const lottery = await lottery749.getLotteryById(lotteryId);
	...
	describe('lottery drawings', async () => {
		it('fulfills drawing', async () => {
			...
			const drawnNumbers = [20, 21, 22, 23, 24, 25, 26];

			await lottery749.fulfill_drawing_tests();

			const lotteryId = await lottery749.lotteryId();
			assert.equal(lotteryId, 2, 'New lottery 2 started');

			let lot1 = await lottery749.getLotteryById(1);
			...
			let jackpot = await lottery749.growingJackpot();
			assert.equal(jackpot.toString(), expectedGrowingJackpot, 'jackpot is correct');

			let admin = await lottery749.getUser({ from: deployer });
		...
		it('one ticket wins', async () => {
			...
			let pickedNumbers = [5, 6, 7, 8, 20, 21, 49];
			...
			let adminBalance = await lottery749.getUser({ from: deployer });

			await lottery749.fulfill_drawing_tests();

			let receipt;
			let gasPrice = await lottery749.withdrawWinnings.estimateGas(winTicketNumber, { from: accounts[4] });
			console.log('Gas Withdraw Winnings = ', gasPrice);
			receipt = await lottery749.withdrawWinnings(winTicketNumber, { from: accounts[4] });
			assert.equal(receipt.logs.length, 1, 'triggers one event');
			...
			await lottery749.withdrawWinnings(winTicketNumber, { from: accounts[4] }).should.be.rejected;
			await lottery749.withdrawWinnings(lostTicketNumber, { from: accounts[4] }).should.be.rejected;

			const lot2 = await lottery749.getLotteryById(2);
			...
			let adminBalanceNew = await lottery749.getUser({ from: deployer });
			assert.equal(adminBalanceNew.toString(), adminBalance.toString(), 'admin balance is correct');
			await lottery749.withdrawBalance(adminBalanceNew, { from: deployer });

			adminBalanceNew = await lottery749.getUser({ from: deployer });
			assert.equal(adminBalanceNew, 0, 'admin balance is correct');

			const jackpot = await lottery749.growingJackpot();
			assert.equal(jackpot.toString(), expectedGrowingJackpot, 'jackpot is correct');
		});
		...
		// 	const lotteryId = await lottery749.lotteryId();
		// 	const minimumPrice = await governance.getMinimumPrice(lotteryAddress);
		// 	let lot = await lottery749.getLotteryById(lotteryId);
		// 	const initialJackpot = lot.jackpot;
		//
		// 	let pickedNumbers = [20, 21, 22, 23, 24, 25, 26];
		//
		// 	await lottery749.enter(pickedNumbers, {
		// 		from: accounts[6],
		// 		value: minimumPrice
		...
		// 	await lottery749.fulfill_drawing_tests();
		//
		// 	lot = await lottery749.getLotteryById(lotteryId);
		...
		//
		// 	await lottery749.withdrawWinnings(lostTicket1, { from: accounts[4] }).should.be.rejected;
		// 	await lottery749.withdrawWinnings(lostTicket2, { from: accounts[5] }).should.be.rejected;
		//
		// 	await lottery749.withdrawWinnings(winnerTicketNumber, { from: deployer }).should.be.rejected;
		//
		// 	let ticket = await tickets.getTicketById(winnerTicketNumber);
		// 	assert.equal(ticket.paidOut.toString(), '0', 'PaidOut is correct');
		//
		// 	const receipt = await lottery749.withdrawWinnings(winnerTicketNumber, { from: accounts[6] });
		//
		// 	assert.equal(receipt.logs.length, 1, 'triggers one event');
		// 	assert.equal(receipt.logs[0].event, 'WithdrawWinnings', 'should be the "WithdrawWinnings" event');
		...
		//
		// 	let growingJackpot = await lottery749.growingJackpot();
		...
		// 	await lottery749.fulfill_drawing_tests();
		//
		// 	const lot = await lottery749.getLotteryById(lotteryId);
		// 	assert.equal(lot.jackpotWinners.toString(), '3', 'jackpotWinners is 3');
		//
		// 	await lottery749.withdrawWinnings(winner1, { from: accounts[6] });
		// 	await lottery749.withdrawWinnings(winner2, { from: accounts[7] });
		// 	const receipt = await lottery749.withdrawWinnings(winner3, { from: accounts[8] });
		//
		// 	assert.equal(receipt.logs.length, 1, 'triggers one event');
		// 	assert.equal(receipt.logs[0].event, 'WithdrawWinnings', 'should be the "WithdrawWinnings" event');
	...
	describe('deposit and withdraw', async () => {
		it('makes a deposit', async () => {
			const receipt = await lottery749.deposit({ from: accounts[1], value: web3.utils.toWei('10', 'Ether') });
			...
			const user1 = await lottery749.getUser({ from: accounts[1] });
			assert.equal(user1, web3.utils.toWei('10', 'Ether'), 'balance is correct');
		});

		it('withdraws balance', async () => {
			await lottery749.withdrawBalance(web3.utils.toWei('0.00001', 'Ether'), { from: accounts[0] }).should.be.rejected;
			await lottery749.withdrawBalance(web3.utils.toWei('0.000001', 'Ether'), { from: accounts[2] }).should.be.rejected;

			const receipt = await lottery749.withdrawBalance(web3.utils.toWei('10', 'Ether'), { from: accounts[1] });
	...
	describe('lottery getter', async () => {
		it('returns lottery draws', async () => {
			...
			const result = await lottery749.getLotteryByIds(ids);
